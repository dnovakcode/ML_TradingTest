#!/usr/bin/env python3
"""
🚀 АВТОНОМНЫЙ ТОРГОВЫЙ БОТ v4.0
Адаптивная система наград с прогрессивным обучением
"""
...
class TradeTracker:
    ...
    def __init__(self):
        ...
        # Статистика для адаптивной системы наград
        self.total_profitable_trades = 0
        self.total_losing_trades = 0
        self.recent_sharpe_ratio = 0.0
    ...
    def open_trade(self, entry_price: float, position_size: float,
        ...
        # Если есть RM, требуем минимум 3 шага удержания (уменьшено с 5)
        min_hold = 3 if (stop_loss > 0 or take_profit > 0) else 0
    ...
    def close_trade(self, exit_price: float, commission: float = 0.0,
        ...
        # Обновляем серии и общую статистику
        ...
        # Проверяем нарушение контрактных обязательств (мягче)
        contract_violated = (trade.steps_held < trade.min_hold_steps and
                           reason == 'manual' and
                           abs(pnl_percent) < 3.0)  # Снижен порог с 5% до 3%
    ...
    def _update_sharpe_ratio(self):
        """Обновить текущий Sharpe ratio на основе последних сделок"""
        if len(self.closed_trades) < 5:
            self.recent_sharpe_ratio = 0.0
            return
        
        recent_trades = self.closed_trades[-20:]  # Последние 20 сделок
        ...
        if len(returns) > 1:
            ...
            if std_return > 0:
                ...
                self.recent_sharpe_ratio = mean_return / std_return
            else:
                self.recent_sharpe_ratio = mean_return * 0.1  # Если нет волатильности
        else:
            self.recent_sharpe_ratio = 0.0
...
class TradingEnvironment(gym.Env):
    """
    Торговое окружение с адаптивной системой наград
    """
    ...
    def __init__(self, initial_balance: float = 10000, detailed_logging: bool = True):
        ...
        # Параметры адаптивной системы наград
        self.total_steps_taken = 0
        self.learning_maturity = 0.0  # От 0 (новичок) до 1 (опытный)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        closed_trades_from_stops = self._check_and_execute_stops()

        # Исполняем новое действие
        new_trade_opened, action_type, closed_trades_from_action = self._execute_action(action)

        # ИСПРАВЛЕНИЕ: Собираем все закрытые сделки для системы наград
        closed_trades = closed_trades_from_stops + closed_trades_from_action

        # Обновляем параметры обучения
        self.total_steps_taken += 1
        self._update_learning_maturity()

        # Рассчитываем награду с учетом стадии обучения
        ...
        info = {
            ...
            'sharpe_ratio': self.trade_tracker.recent_sharpe_ratio,
    ...
    def _update_learning_maturity(self):
        """Зрелость обучения зафиксирована - никаких урезаний наград"""
        # Отключаем систему постепенного урезания наград
        # Зрелость всегда остается на минимуме для максимальных наград
        self.learning_maturity = 0.0  # Постоянно новичок = постоянно максимальные награды

    def _calculate_adaptive_reward(self, prev_value: float, current_value: float,
                                  action: np.ndarray, closed_trades: List[Dict],
                                  new_trade_opened: bool, action_type: str) -> float:
        """
        ПРОСТАЯ СИСТЕМА НАГРАД
        Убыток = -1, Ноль = 10, Прибыль = 50
        """
        total_reward = 0.0
        self.last_reward_breakdown = {
            'trade_result': 0.0,
        ...
        # === НАГРАДА ТОЛЬКО ЗА РЕЗУЛЬТАТЫ ЗАКРЫТЫХ СДЕЛОК ===
        if closed_trades:
            if self.detailed_logging:
                print(f"💰 ОБРАБАТЫВАЮ {len(closed_trades)} закрытых сделок для наград:")

            for i, trade in enumerate(closed_trades):
                pnl_after_commission = trade['pnl_after_commission']

                if self.detailed_logging:
                    print(f"  [{i+1}] PnL: ${pnl_after_commission:+.2f} | Причина: {trade.get('close_reason', 'unknown')}")
                ...
                if pnl_after_commission > 0:
                    ...
                    trade_reward = 50
                    if self.detailed_logging:
                        print(f"      🎉 НАГРАДА: +50 за прибыль ${pnl_after_commission:+.2f}")
                elif pnl_after_commission == 0:
                    # Безубыток = 10
                    trade_reward = 10
                    if self.detailed_logging:
                        print(f"      💪 НАГРАДА: +10 за безубыток")
                else:
                    # Любой убыток = +1
                    trade_reward = 1
                    if self.detailed_logging:
                        print(f"      🔄 НАГРАДА: +1 за попытку (убыток ${pnl_after_commission:+.2f})")
    ...
    def _execute_action(self, action: np.ndarray) -> Tuple[bool, str, List[Dict]]:
        """
        Исполнение действия с детальным логированием
        Возвращает (новая_сделка_открыта, тип_действия, закрытые_сделки)
        тип_действия: 'trade', 'hold', 'skip_buy', 'skip_sell', 'skip_invalid'
        закрытые_сделки: список закрытых сделок для системы наград
        """
        ...
        if position_ratio > 0.05:  # Минимум 5% для открытия позиции
            if self.balance <= 50:  # Недостаточно средств
                if self.detailed_logging:
                    print(f"⚪ HOLD: Недостаточно средств ${self.balance:.2f} < $50")
                return False, 'hold', []

            desired_amount = portfolio_value * abs(position_ratio)
            max_buy_amount = min(self.balance * 0.99, desired_amount)  # Оставляем 1% на комиссию
            ...
            if max_buy_amount + commission > self.balance:
                if self.detailed_logging:
                    print(f"⚪ HOLD: Недостаточно для покупки с комиссией")
                return False, 'hold', []
            ...
            maturity_info = f" [LM:{self.learning_maturity:.1%}]" if self.learning_maturity < 0.5 else ""
            print(f"🟢 LONG: {btc_to_buy:.6f} BTC @ ${self.current_price:.2f}{risk_info}{maturity_info}")
            return True, 'trade', []
        ...
        # ПРОДАЖА (закрытие позиции)
            ...
            if self.btc_amount <= 0:
                if self.detailed_logging:
                    print(f"⚪ HOLD: Нет BTC для продажи")
                return False, 'hold', []

            sell_ratio = min(abs(position_ratio), 1.0)
            btc_to_sell = min(self.btc_amount, self.btc_amount * sell_ratio)

            if btc_to_sell <= 0.0001:  # Микро-остатки
                if self.detailed_logging:
                    print(f"⚪ HOLD: Слишком мало BTC для продажи {btc_to_sell:.6f}")
                return False, 'hold', []
            ...
            if closed_info:
                ...
                maturity_info = f" [LM:{self.learning_maturity:.1%}]" if self.learning_maturity < 0.5 else ""
                print(f"🔴 CLOSE {len(closed_info)} trades @ ${self.current_price:.2f} | Avg PnL: {avg_pnl:+.2f}%{maturity_info}")

            # ВАЖНО: Возвращаем информацию о закрытых сделках для системы наград!
            return False, 'trade', closed_info
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        if self.trade_tracker.closed_trades:
            ...
            # Sharpe ratio
            obs.append(min(10, max(-10, self.trade_tracker.recent_sharpe_ratio)))
        else:
            obs.extend([0, 0, 0, 0])
        ...
        # Уровень обучения
        obs.append(self.learning_maturity)
...
class TradingAgent:
    ...
    def __init__(self, env: TradingEnvironment, model_path: Optional[str] = None):
        self.env = env  # Сохраняем ссылку на среду для логирования
        ...
        if model_path and os.path.exists(model_path):
            ...
            self.model = SAC(
                "MlpPolicy",
                env,
                learning_rate=0.0001,  # ПОНИЖЕН для более стабильного обучения
                buffer_size=100000,
    ...
    def learn_online(self, obs: np.ndarray, action: np.ndarray,
        ...
        if self.model.replay_buffer.size() >= self.model.learning_starts:
            try:
                ...
                # Q-values диагностика каждые 50 обучений
                if self.learning_stats['total_updates'] % 50 == 0:
                    self._log_q_values_diagnostics(obs, action)
...
└─ 🎯 Лучший ROI: {self.learning_stats.get('best_roi', 0):.2%}
    ...
    def _log_q_values_diagnostics(self, obs, action):
        """Диагностика Q-values для понимания 'мыслей' модели"""
        try:
            import torch

            # Если action - это tuple (из predict), берем только первый элемент
            if isinstance(action, tuple):
                action = action[0]

            # Преобразуем в тензоры
            obs_tensor = torch.FloatTensor(obs).unsqueeze(0)  # Добавляем batch dimension
            action_tensor = torch.FloatTensor(action).unsqueeze(0)

            # Получаем Q-values от critic сети
            with torch.no_grad():
            ...
            # Тестируем разные действия для сравнения
            test_actions = [
                np.array([0.0, 0.0, 0.0], dtype=np.float32),    # HOLD
                np.array([0.5, 0.1, 0.2], dtype=np.float32),   # Умеренная покупка
                np.array([-0.9, 0.0, 0.0], dtype=np.float32),  # Агрессивная продажа
            ]

            test_q_values = []
            for test_action in test_actions:
                test_action_tensor = torch.FloatTensor(test_action).unsqueeze(0)
                with torch.no_grad():
                    test_q = self.model.critic(obs_tensor, test_action_tensor).mean().item()
                    test_q_values.append(test_q)

            print(f"""
🧠 Q-VALUES ДИАГНОСТИКА (#{self.learning_stats['total_updates']}):
├─ 🎯 Текущее действие: Q={q_mean:.2f} (min:{q_min:.2f}, max:{q_max:.2f})
├─ 🟢 HOLD [0.0,0.0,0.0]: Q={test_q_values[0]:.2f}
├─ 🟡 Умеренная покупка [0.5,0.1,0.2]: Q={test_q_values[1]:.2f}
└─ 🔴 Агрессивная продажа [-0.9,0.0,0.0]: Q={test_q_values[2]:.2f}
            """)

        except Exception as e:
            print(f"⚠️ Ошибка Q-values диагностики: {e}")
...
══════════════════════════════════
💰 Баланс: $10,000
🎯 Система наград: Адаптивная с прогрессивным обучением
🧠 ИИ: SAC с полной свободой
📈 Фокус: Sharpe ratio и risk-adjusted returns
══════════════════════════════════
        """)
    ...
    def _log_step(self, info: Dict, action: np.ndarray, reward: float):
        ...
        if self.total_steps % 10 == 0:
            ...
            maturity = info.get('learning_maturity', 0.0)
            ...
            if reward_details:
                ...
                if reward_details.get('exploration_bonus', 0) != 0:
                    components.append(f"Explore:{reward_details['exploration_bonus']:+.0f}")
                if reward_details.get('risk_management', 0) != 0:
                    components.append(f"RM:{reward_details['risk_management']:+.0f}")
                if reward_details.get('learning_bonus', 0) != 0:
                    components.append(f"Learn:{reward_details['learning_bonus']:+.0f}")
                if reward_details.get('skip_penalty', 0) != 0:
                    components.append(f"Skip:{reward_details['skip_penalty']:+.0f}")
            ...
            print(f"""
[{current_time}] 📊 Шаг {self.total_steps} {stage_emoji}
├─ 💰 Портфель: ${portfolio:,.2f} (ROI: {roi:+.2f}%)
├─ 📈 Sharpe: {sharpe:.2f} | Maturity: {maturity:.1%}
├─ 🎯 Действие: {action_str}
├─ 🏆 Награда: {reward_str}
...
📊 ROI: {total_roi:+.2f}%
📈 Sharpe Ratio: {self.env.trade_tracker.recent_sharpe_ratio:.2f}
...
🎓 Learning Maturity: {self.env.learning_maturity:.1%}
═════════════════════════════════════
        """)
